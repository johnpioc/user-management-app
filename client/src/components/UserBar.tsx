...
export default function UserBar({ userBarOpen, setUserBarOpen }: UserBarProps) {
  ...
  return (
    <section
      className={`h-screen w-screen transition-all ease-in-out duration-400 fixed
            top-0 left-0
    ...
    >
      ...
      >
        {/* EXIT BUTTON */}
        <div
          className="h-8 lg:h-full w-64 lg:w-8 flex justify-center   
                        items-center"
        >
          <button
            className={`bg-sky-600 w-full h-full lg:h-40 text-white 
        ...
        <div
          className="flex flex-col justify-start items-start text-black bg-white
                        h-[70vh] lg:h-full w-full lg:w-xl p-4 text-lg"
        >
          {/* TITLE */}
          <h2 className="text-2xl text-sky-600 font-bold">{mode} USER</h2>

          {/* NAME */}
          <label className="font-bold mt-8" htmlFor="name">
            Name
          </label>
          {mode == "VIEW" ? (
            <p>{user.name}</p>
            ...
            <input
              className="w-full rounded-md border-slate-300 border-2 p-1 
                                    mt-1"
              type="text"
              id="name"
              value={user.name}
          ...
          {/* ROLE */}
          <label className="font-bold mt-4" htmlFor="role">
            Role
          </label>
          {mode == "VIEW" ? (
            <p>{user.role}</p>
          ...
          {/* STATUS */}
          <label className="font-bold mt-4" htmlFor="status">
            Status
          </label>
          {mode == "VIEW" ? (
            <p>{user.status}</p>
          ...
          <div className="flex justify-center items-center w-full space-x-4 mt-8">
            ...
            {mode != "VIEW" && (
              <button
                className="bg-sky-600 rounded-md px-8 py-2
            ...
            {mode == "VIEW" && (
              <button
                className="bg-red-500 rounded-md px-8 py-2 
                                        text-white font-bold cursor-pointer"
