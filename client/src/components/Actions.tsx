...
const isEmptyFilter = (filter: Filter): boolean => {
  ...
  return (
    filter.startDate == new Date(0).toISOString().split("T")[0] &&
...
export default function Actions({
  ...
  return (
    <section
      className="w-full lg:w-4xl flex flex-col lg:flex-row space-y-4 lg:space-y-0
            justify-between items-center text-lg mb-4"
    >
      <div
        className="flex flex-col lg:flex-row items-center justify-center lg:space-x-4 
                space-y-4 lg:space-y-0 w-full lg:justify-start"
      >
        {/* SEARCH BAR */}
        <div
          className="w-full lg:w-96 rounded-full flex space-x-2 
        ...
        >
          ...
          />
          <button
            className={`bg-sky-600 overflow-hidden rounded-md text-sm
                            text-white ${
                              searchText != filter.nameContainsChars
                                ? "w-20"
                                : "w-0"
        ...
        {/* FILTER BUTTON */}
        <button
          className={`lg:p-2 p-0 shadow-xl rounded-full border-2 flex justify-center 
      ...
      {/* ADD USER BUTTON */}
      <button
        className="rounded-full flex justify-center items-center space-x-2
