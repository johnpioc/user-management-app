...
export default function UserTable({ setUserBarOpen }: UserTableProps) {
  ...
  return (
    <section
      className="w-full lg:w-4xl rounded-lg overflow-hidden shadow-xl 
            bg-white relative z-2"
    >
      {/* TABLE HEADERS */}
      <div
        className="grid lg:grid-cols-4 grid-cols-2 text-white font-bold text-lg
                    bg-sky-600 p-2"
      >
        <p>Name</p>
        <p>Role</p>
      ...
      {users.map((user, index) => (
        <div
          ...
          className={`grid lg:grid-cols-4 grid-cols-2 
                        ...
                        ${
                          index % 2 == 1 ? "bg-slate-100" : "bg-white-200"
                        } cursor-pointer`}
